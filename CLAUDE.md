...
- [ ] Tapping the Core orb on a phone triggers the world-flip → chrome-bust reveal (the live tuned mobile bust), and it animates smoothly without jank.
- [ ] Tilting the phone makes the bust's head look around (gaze follows tilt); a deliberate tilt flick boots the 3D face screen; on iOS the motion-permission prompt appears on the first orb tap; denied/unsupported degrades to a calm idle drift (no errors).
